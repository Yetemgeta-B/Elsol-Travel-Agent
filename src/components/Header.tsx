...
const Header = () => {
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-4 mx-6 mt-6 rounded-full ${isScrolled ? 'py-3 backdrop-blur-md bg-black/30 border border-elsol-sage/10' : 'py-5 bg-transparent'}`}>
      <div className="container mx-auto px-4 md:px-8 max-w-screen-2xl flex flex-wrap items-center justify-between">
        ...
        <div className={`hidden md:flex items-center space-x-6 text-sm w-full text-right justify-end transition-all duration-300 ${isScrolled ? 'h-0 opacity-0 mb-0 overflow-hidden' : 'h-auto opacity-100 mb-3'}`}>
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="md:hidden">
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 transition-colors text-gray-300" aria-label="Toggle menu">
        ...
        <div className={`md:hidden w-full transition-all duration-300 ease-in-out overflow-hidden ${mobileMenuOpen ? 'max-h-screen opacity-100 mt-5' : 'max-h-0 opacity-0'}`}>
          <nav className="flex flex-col space-y-4 py-4 backdrop-blur-md bg-black/70 rounded-2xl px-4 mt-2">
            <Link to="/#home" className="font-medium px-2 py-1 text-gray-300 hover:text-elsol-sage transition-colors flex items-center" onClick={() => setMobileMenuOpen(false)}>
              <NavigationIcon size={16} className="mr-1 text-elsol-sage" />
              Home
            </Link>
            ...
            <div className="flex flex-col space-y-2 pt-4 border-t border-gray-700">
              <a href="tel:+251911234567" className="flex items-center text-sm text-gray-300 hover:text-elsol-sage transition-colors">
                <Phone size={14} className="mr-2" /> +251 911 234 567
              </a>
              <a href="mailto:info@elsoltravel.com" className="flex items-center text-sm text-gray-300 hover:text-elsol-sage transition-colors">
                <Mail size={14} className="mr-2" /> info@elsoltravel.com
              </a>
            </div>
          </nav>
